            ...
            weights = NULL, parallel = TRUE, nWorkers = 2, verbose=0, indent=0)
{
  ...
  if (length(weights) > 0)
    dimnames(weights) = dimnames(counts);
...
)
{
   ...
   if (forceCalculation || is.null(intermediateResults$combinedDESeqResults))
   {
     ...
     if (!is.null(geneAnnotation.end)) {
       combinedDESeqResults.ext = lapply(combinedDESeqResults, data.frame, geneAnnotation.end, check.names = FALSE);
     } else 
     ...
     for (cri in 1:nUseCombineResults) for (tt in 1:nEnrichmentThresholds) 
        if (length(enrichment[[cri]][[tt]]$CombinedCollection) > 0)
        {
          enrDir1 = multiGSub(c("%combine", "%threshold"), 
                             c(enrichment.useResultsCombinations[cri], enrichment.useThresholds[tt]), enrichmentDir);
