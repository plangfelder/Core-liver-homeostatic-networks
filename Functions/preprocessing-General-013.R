                     ...
                     verbose = 1, ...)
{
  ...
  out = goodGenes(x, verbose = 0) & colSums(x >= minValue, na.rm = TRUE) >= minProportion * ns;
...
conditionalWriteAndAddToAnalysis = function(
  ...
  amAnalysis, amName, allowNonstandardObjectType = FALSE,
  verbose = 0, indent = 0)
{
...

{
   ...
   if (adj.calculateSVAFactors)
   {
     ...
     svaData = sva(t(expr.forFactorAnalysis), mod = model, mod0 = model0);
     printFlush("");
