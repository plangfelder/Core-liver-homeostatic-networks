...
.preprocessing.General.version = "13.00";
...
)
{
  if (!is.null(PCs))
  {
    removedCovariates = c(removedCovariates, colnames(PCs));
  ...
  useEB = method=="EB";
  removedIndex = match(removedCovariates, colnames(covariates));
  if (any(is.na(removedIndex)))
    stop("The following entries in 'removedCovariates' were\n ",
         "not found among colnames(covariates):\n",
         paste(removedCovariates[is.na(removedIndex)], collapse = ", "))

  if (length(retainedCovariates)>0)
  {
    retainedIndex = match(retainedCovariates, colnames(covariates));
    ...
    retainedCovariates = covariates[, retainedIndex];
  } else 
    retainedCovariates = NULL;
  ...
  if (useEB)
  {
    out = ebData$adjustedData
  } else
    out = ebData$adjustedData.OLS
...
preprocessGeneral = function(
   ...
   saveCollapsed = TRUE,
   exprDir.OSR.collapsed = exprDir.OSR,
   ...
   exprDir.IORep.collapsed = exprDir.IORep,
   ...
   exprDir.IORem.collapsed = exprDir.IORem,
   ...
   exprDir.IORep.adj.collapsed = exprDir.IORep.adj,
   ...
   exprDir.IORem.adj.collapsed = exprDir.IORem.adj,
...
)
{
                     ...
                     pointLabels = NULL)
   {
     if (length(pca.correctForColumn) > 0)
     {
       ...
       data.corr = residuals(lm(data~colData));
   ...
   {
     if (is.numeric(adj.fitToSamples)) 
     {
       tmp = adj.fitToSamples;
       adj.fitToSamples = rep(FALSE, nSamples);
       adj.fitToSamples[tmp] = TRUE;
     }

     if (length(adj.fitToSamples)!=nSamples)
       stop("When given, 'adj.fitToSamples' should be logical with length equal number of samples in 'xdata'.");
   }
   ...
   if (length(intermediateResults$entrez)==0 || forceCalculation)
   {
     ...
     intermediateResults$entrez$exprDir.entrez = exprDir.entrez
     intermediateResults$entrez$xdataFile.entrez = xdataFile.entrez;
     intermediateResults$entrez$dataFile.entrez = dataFile.entrez;
   ...
   while (changed)
   {
      ...
      if (length(HE.VST.OR[[ORStep]]$OSR)==0 || forceCalculation)
      {
         ...
         if (length(adj.fitToSamples)>0) adj.fitToSamples = adj.fitToSamples[!outliers]
      ...
      if (length(adj.removedCovariates) > 0)
      {
        plotSampleTreeFromData(expr.adj[, HE.VST.OR[[ORStep]]$OSR$or.topProbes], scale = or.scaleData, colors.x,
                    ...
                    sub = "adjusted data", dendroLabels = if (stp.includeSampleLabels) NULL else FALSE);
      ...
      plotSampleTreeFromData(expr.OSR[, HE.VST.OR[[ORStep]]$OSR$or.topProbes], scale = or.scaleData, phenoColors.OSR,
                  ...
                  sub = "adjusted data, outlier samples removed",
   ...
   if (saveOSR)
   {
     ...
     intermediateResults$OSRFiles = list(
           exprDir.OSR = exprDir.OSR,
           xdataFile.OSR = xdataFile.OSR,
           exprFile.OSR = exprFile.OSR,
           sampleDir.OSR = sampleDir.OSR,
           sampleFile.OSR = sampleFile.OSR,
           dataFile.OSR = dataFile.OSR);
     if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
   ...
   if (ior.replace || ior.remove || saveIORWeights)
   {
      if (length(intermediateResults$IOR)==0 || forceCalculation)
      {
        ...
        haveIORWeights = TRUE;
        intermediateResults$IORFiles = list();
        if (saveIORWeights)
        {
           ...
           intermediateResults$IORFiles$IORWeightDir = IORWeightDir;
           intermediateResults$IORFiles$xdataWeightFile.IORep = xdataWeightFile.IORep;
           intermediateResults$IORFiles$weightFile.IORep = weightFile.IORep;
           intermediateResults$IORFiles$xdataWeightFactorFile.IORep = xdataWeightFactorFile.IORep
        }
        ...
        if (ior.replace)
        {
          ...
          if (saveIOR)
          {
             ...
             intermediateResults$IORFiles$exprDir.IORep = exprDir.IORep;
             intermediateResults$IORFiles$xdataFile.IORep = xdataFile.IORep;
             intermediateResults$IORFiles$exprFile.IORep = exprFile.IORep;
             intermediateResults$IORFiles$dataFile.IORep = dataFile.IORep;
          }
        }
        ...
        if (ior.remove)
        {
          ...
          if (saveIOR)
          {
             ...
             if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
                ...
                allowNonstandardAnalysisType = TRUE,
                analysis = amAnalysis, verbose = verbose - 1);
             intermediateResults$IORFiles$exprDir.IORem = exprDir.IORem;
             intermediateResults$IORFiles$xdataFile.IORem = xdataFile.IORem
             intermediateResults$IORFiles$exprFile.IORem = exprFile.IORem;
             intermediateResults$IORFiles$dataFile.IORep = dataFile.IORep;
          }
        }
        forceCalculation = TRUE;
      ...
      if (forceCalculation || length(intermediateResults$IOR.adj)==0)
      {
        ...
        intermediateResults$IOR.adjFiles = list();
        ...
        if (adj.removeNPCs > 0)
        {
          ...
          for (type in c("replaced", "removed")[c(ior.replace, ior.remove)]) 
          {
             ...
             if (saveIOR.adj) 
             {
               intermediateResults$IOR.adjFiles$PCDir.IOR = sampleDir.OSR;
               if (type=="removed") intermediateResults$IOR.adjFiles$PCFile.IORem = PCFile.IORem else
                     intermediateResults$IOR.adjFiles$PCFile.IORep = PCFile.IORep;
             }
          }
        } else
		...
		file = file.path(exprDir.IORem.adj, dataFile.IORem.adj));
	     ...
	     intermediateResults$IOR.adj$dataFile.IORem.adj = dataFile.IORem.adj;
             amAnalysis = am.addMultipleObjectsToAnalysis(
                ...
                allowNonstandardAnalysisType = TRUE,
                analysis = amAnalysis, verbose = verbose - 1);
              intermediateResults$IOR.adjFiles$exprDir.IORem.adj = exprDir.IORem.adj;
              intermediateResults$IOR.adjFiles$exprFile.IORem.adj = exprFile.IORem.adj;
              intermediateResults$IOR.adjFiles$dataFile.IORem = dataFile.IORem;
           }
           if (ior.replace)
           {
		...
		file = file.path(exprDir.IORep.adj, dataFile.IORep.adj));
	     ...
	     intermediateResults$IOR.adj$dataFile.IORep.adj = dataFile.IORep.adj;
             amAnalysis = am.addMultipleObjectsToAnalysis(
                ...
                allowNonstandardAnalysisType = TRUE,
                analysis = amAnalysis, verbose = verbose - 1);
              intermediateResults$IOR.adjFiles$exprDir.IORep.adj = exprDir.IORep.adj;
              intermediateResults$IOR.adjFiles$exprFile.IORep.adj = exprFile.IORep.adj;
              intermediateResults$IOR.adjFiles$dataFile.IORep = dataFile.IORep;
           }
	}
        forceCalculation = TRUE;
      } else {
   ...
   if ((length(cl.probe2gene) > 0) && (forceCalculation || length(intermediateResults$collapse)==0))
   {
     ...
     intermediateResults$collapseFiles = list();
     if (saveCollapsedIORWeights)
     {
       ...
       amAnalysis = am.addMultipleObjectsToAnalysis(
            ...
            allowNonstandardAnalysisType = TRUE,
            analysis = amAnalysis, verbose = verbose - 1);
       intermediateResults$collapseFiles$IORWeightDir.collapsed = IORWeightDir.collapsed;
       intermediateResults$collapseFiles$xdataWeightFile.IORep.collapsed = xdataWeightFile.IORep.collapsed;
       intermediateResults$collapseFiles$xdataWeightFactorFile.IORep.collapsed = xdataWeightFactorFile.IORep.collapsed;
       intermediateResults$collapseFiles$weightFile.IORep.collapsed = weightFile.IORep.collapsed;
     } else {
     ...
     if (saveCollapsed)
     {
       ...
       if ("OSR" %in% names(collapsedXData))
       {
         ...
         intermediateResults$collapseFiles$exprDir.OSR.collapsed = exprDir.OSR.collapsed;
         intermediateResults$collapseFiles$xdataFile.OSR.collapsed = xdataFile.OSR.collapsed;
       }
       if ("OSR" %in% names(collapsedExpr))
       {
         ...
         intermediateResults$collapseFiles$exprDir.OSR.collapsed = exprDir.OSR.collapsed;
         intermediateResults$collapseFiles$exprFile.OSR.collapsed = exprFile.OSR.collapsed;
       }
       ...
       { 
         ...
         intermediateResults$collapse$xdataFile.OSR.collapsed = xdataFile.OSR.collapsed;
         intermediateResults$collapse$exprFile.OSR.collapsed = exprFile.OSR.collapsed;
         ...
         if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
           ...
           allowNonstandardAnalysisType = TRUE,
           analysis = amAnalysis, verbose = verbose - 1);
         intermediateResults$collapseFiles$dataFile.OSR.collapsed = dataFile.OSR.collapsed;
       }
     }
     ...
     if (saveCollapsed && ior.remove)
     {
        ...
        if ("IORemoved" %in% names(collapsedXData))
        {
          ...
          intermediateResults$collapseFiles$exprDir.IORem.collapsed = exprDir.IORem.collapsed;
          intermediateResults$collapseFiles$xdataFile.IORem.collapsed = xdataFile.IORem.collapsed;
        }
        if ("IORemoved" %in% names(collapsedExpr))
        {
          ...
          intermediateResults$collapseFiles$exprDir.IORem.collapsed = exprDir.IORem.collapsed;
          intermediateResults$collapseFiles$exprFile.IORem.collapsed = exprFile.IORem.collapsed;
        }
        if (length(saveList) > 0)
        {
          saveList = c(saveList, "sampleAnnotation.OSR");
          ...
          if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
             ...
             allowNonstandardAnalysisType = TRUE,
             analysis = amAnalysis, verbose = verbose - 1);
          intermediateResults$collapseFiles$dataFile.IORem.collapsed = dataFile.IORem.collapsed;
        }
     }
     ...
     if (saveCollapsed && ior.replace)
     {
        ...
        if ("IOReplaced" %in% names(collapsedXData))
        {
          ...
          intermediateResults$collapseFiles$exprDir.IORep.collapsed = exprDir.IORep.collapsed;
          intermediateResults$collapseFiles$xdataFile.IORep.collapsed = xdataFile.IORep.collapsed;
        }
        if ("IOReplaced" %in% names(collapsedExpr))
        {
          ...
          intermediateResults$collapseFiles$exprDir.IORep.collapsed = exprDir.IORep.collapsed;
          intermediateResults$collapseFiles$exprFile.IORep.collapsed = exprFile.IORep.collapsed;
        }
        if (length(saveList) > 0)
        {
          saveList = c(saveList, "sampleAnnotation.OSR");
          ...
          if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
             ...
             allowNonstandardAnalysisType = TRUE,
             analysis = amAnalysis, verbose = verbose - 1);
          intermediateResults$collapseFiles$dataFile.IORep.collapsed = dataFile.IORep.collapsed;
        }
     }
     ...
     if (saveCollapsed && "IORep.adj" %in% names(collapsedExpr))
     {
       ...
       if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
          ...
          allowNonstandardAnalysisType = TRUE,
          analysis = amAnalysis, verbose = verbose - 1);
       intermediateResults$collapseFiles$exprDir.IORep.adj.collapsed = exprDir.IORep.adj.collapsed;
       intermediateResults$collapseFiles$exprFile.IORep.adj.collapsed = exprFile.IORep.adj.collapsed;
       intermediateResults$collapseFiles$dataFile.IORep.adj.collapsed = dataFile.IORep.adj.collapsed;
     }
     ...
     if (saveCollapsed && "IORem.adj" %in% names(collapsedExpr))
     {
       ...
       if (addToAnalysisManager) amAnalysis = am.addMultipleObjectsToAnalysis(
          ...
          allowNonstandardAnalysisType = TRUE,
          analysis = amAnalysis, verbose = verbose - 1);
       intermediateResults$collapseFiles$exprDir.IORem.adj.collapsed = exprDir.IORem.adj.collapsed;
       intermediateResults$collapseFiles$exprFile.IORem.adj.collapsed = exprFile.IORem.adj.collapsed;
       intermediateResults$collapseFiles$dataFile.IORem.adj.collapsed = dataFile.IORem.adj.collapsed;
     }
     forceCalculation = TRUE;
