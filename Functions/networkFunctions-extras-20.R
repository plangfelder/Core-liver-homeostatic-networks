...
#=================================================================================================
#
# rbind disregarding column names
#
#=================================================================================================
...
splitData = function(data, split, nameBase = "", includeInAllSets = NULL)
{
  ...
  for (g in 1:nGroups)
  {
     splitData[[g]] = list(data = data[union(which(split==groups[g]), includeInAllSets), ]);
  }
...
standardizeSampleNames = function(s, extreme = FALSE) 
{
  out = multiGSub(c("^X", " +", "-+", "\\.+", "\\+"), c("", "_", "_", "_", "_"), s);
...
# idea is similar to Seurat that scales each cell by the total count. Here I want to have the option of excluding a few of
# the highest counts in each cell.

normalizeByTotalCount = function(x, excludeTop = 0)
{
  xx = x;
  if (excludeTop > 0)
  {
    nGenes = nrow(x)
    p = 1-excludeTop/ nGenes;
    q = rowQuantileC(x, p);
    xx[xx >= q] = 0;
  }
  sums = rowSums(xx);
  meanSum = exp(mean(log(sums)));
  factors = meanSum/sums;
  out = x * factors;
  attr(out, "truncatedSums") = sums;
  out;
}
  ...
  xlim = NULL, ylim = NULL, ...)
{
  ...
  plot(umapData$x[base], umapData$y[base],
     xlab = "", ylab = "", xaxt = "n", yaxt = "n",
     cex = cex.base[base], pch = pch.base, col = baseCol[base], bg = baseBg[base], lwd= lwd.base[base],
     main = main,
