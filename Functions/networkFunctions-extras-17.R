...
placeLegend = function(points.x, points.y, nPositions = 5,
                       ...
                       ...)
{
  ...
  for (nc in 1:nTryNCol)
  {
    ...
    plotBox = limitBox;
    xRange = plotBox[2] - plotBox[1];
    yRange = plotBox[4] - plotBox[3];
    tryPositions.x[[nc]] = seq(from = plotBox[1], to = plotBox[2] - legendBox[[nc]]$w, length.out = nPositions);
    tryPositions.y[[nc]] = seq(from = plotBox[3], to = plotBox[4] - legendBox[[nc]]$h, length.out = nPositions);
    for (tpx in 1:nPositions) for (tpy in 1:nPositions)
    {
      ...
      stat[tpy, tpx, nc] = nPoints * 2*(xRange + yRange)^2 *
...
legendClean = function(x, y=NULL, ..., ncol = 1, points.x = NULL, points.y = NULL, nPositions = 5,
...
addScatterplotCounts = function(
  ...
  prefix = "n=",
  ...,
  nPositions = 5, tryNCol = c(1, 1 + addFisherTestPValues), pointSize = 0.02, maxOverlap = 10,
  ...
  )
{
  ...
  while (it <= maxIterations)
  {
    ...
    x = x + shift[, 1];
    y = y + shift[, 2];
    if (plotProgress) points(x, y, pch = 21, bg = point.bg);
    it = it + 1;
...
stretchLim = function(x, rx)
{
  minx = min(x, na.rm = TRUE);
  maxx = max(x, na.rm = TRUE);
  c(minx - rx * (maxx-minx), maxx + rx * (maxx-minx));
}
                             ...
                             
{
  ...
  plotDendrogram(tree.rev, labels = FALSE, hang = -1, horiz = TRUE, reverseDirection = FALSE, 
                 adjustRange = TRUE);
  par(mar = mar.main);
  text0 =  round(corMat[order1, order1], roundTextToDigits);
  text = text0;
  if (!is.null(pValues))
  {
    if (pValuesBelow) text = spaste(text, "\n", signif(pValues[order1, order1], 1)) else
        text = spaste(text, " (", signif(pValues[order1, order1], 1), ")");
    par(lheight = 0.85);
  }
  text[abs(text0) < showTextAbove] = "";
...
heatmapWithDendro = function(
  ...
  ySymbols = NULL,
  colors = blueWhiteRed(100),
  zlim = c(-max(abs(mat), na.rm = TRUE), max(abs(mat), na.rm = TRUE)),
  colors.lab.x = 1, colors.lab.y = 1, 
  colTree.shift = 0.5,
  main = "", ...)

{
  ...
  if (showText)
  {
    if (is.null(text))
    {
      text =  round(mat, roundTextToDigits);
      text[abs(text) < showTextAbove] = "";
    } 
  }
  labeledHeatmap(mat,
               xLabels = xLabels,
...
associationScreening.singleDesign = function(
   ...
   getCoeffSignificance = NULL,

   responseNames = colnames(response),
   addTestNameToColumnNames = TRUE,
   testNameSep = ".for.",
   modelComparisonSuffix = if (getCoeffSignificance) ".modelComp" else 
                              spaste(".for.", paste(testNames, collapse = "|")),
   ...
   verbose = 2, indent = 0)
{
  ...
  if (haveReduced)
  {
    ...
    if (is.null(termsOfInterest)) termsOfInterest = setdiff(terms.full, terms.reduced);
    p.common = rep(NA, nResponses) 
  } else {
    if (is.null(termsOfInterest)) 
       stop("'termsOfInterest' must be explicitly given when 'reduced' is NULL.");
  }
  if (is.null(getCoeffSignificance)) getCoeffSignificance = !haveReduced;
  ...
  if (!all(termsOfInterest %in% colnames(predictorModelMat)))
    stop("Some 'termsOfInterest' are not among column names of model matrix:\n  ",
         paste(termsOfInterest[!termsOfInterest %in% colnames(predictorModelMat)], collapse = ", "));

  if (autoRemoveDependentCovariates)
  {
    nPred = ncol(predictorModelMat);
    termIndex = match(termsOfInterest, colnames(predictorModelMat));
  ...
  termStatNames = termStatNames.associationScreening(getQvalues =getQvalues,
                                      ...
                                      getDoF = getDoF, getCLS = getCLS);

  outColIndex = match(termStatNames.associationScreening(getQvalues = FALSE, getFDR = FALSE, 
                     getZstatistics = FALSE, getDoF = FALSE, getCLS = FALSE), termStatNames);

  stats.fromFits = array(NA, dim = c(nResponses, 3, nTerms));
  ...
  for (r in 1:nResponses)
  {
    ...
    if (is.finite(sd1) && sd1 > 0)
    {
      ...
      try({
        ...
        rows = match(termsOfInterest, rownames(coeffs));
        stats.fromFits[r, , ] = t(coeffs[rows, c(1,3,4), drop = FALSE]);
      })
    }
  ...
  for (t in 1:nTerms)
  {
    ...
    if (getCoeffSignificance)
    {
      Z1 = sign(stats.out[[t]][, coeffCol]) * qnorm(stats.out[[t]] [, pCol]/2, lower.tail = FALSE);
      ...
      stats.out[[t]] = stats.out[[t]] [, coeffCol, drop = FALSE];
    }
  }
  ...
  if (haveReduced)
  {
    commonStatNames = commonStatNames.associationScreening(getQvalues =getQvalues,
                    getFDR =getFDR, getZstatistics = getZstatistics, getDoF = getDoF, getCLS = getCLS);
    stats.out.common = matrix(NA, nResponses, nCommonStatistics.associationScreening(getQvalues =getQvalues,
                    getFDR =getFDR, getZstatistics = getZstatistics, getDoF = getDoF, getCLS = getCLS));
    colnames(stats.out.common) = spaste(commonStatNames, modelComparisonSuffix);
    rownames(stats.out.common) = responseNames;

    pCol = match("p", commonStatNames);
    qCol = match("q", commonStatNames);
    zCol = match("Z", commonStatNames);
    fdrCol = match("FDR", commonStatNames);
    DoFCol = match("DoF", commonStatNames);
    CLSCol = match("CLS", commonStatNames);

    stats.out.common[, pCol] = p.common;

    # For Z: we take the coefficient of the last term in the results...
    if (getZstatistics)
      stats.out.common [, zCol] = sign(stats.out[[t]][, coeffCol]) *
             qnorm(p.common/2, lower.tail = FALSE);
    if (getQvalues)
        stats.out.common [, qCol] = qvalue.restricted(p.common);

    if (getFDR)
        stats.out.common [, fdrCol] = p.adjust(p.common, method = "fdr");

    if (getDoF)
        stats.out.common [, DoFCol] = nDoF;

    if (getCLS)
        stats.out.common [, CLSCol] = correlationLikeStatistic(Z = stats.out.common [, zCol], n = nDoF);
  }
  ...
  if (flatOutput)
  {
    ...
    if (haveReduced) out = data.frame(out, stats.out.common);
  } else {
    out = c(stats.out, if (haveReduced) list(modelComparison = stats.out.common) else NULL);
  }
   ...
   verbose = 2, indent = 0)
{
  ...
  for (ds in 1:nDesigns)
  {
    ...
    as.sd[[ds]] = associationScreening.singleDesign(
       ...
       getCoeffSignificance = NULL,  # Code below assumes that for each design there is only one 
                                     # column of pvalues and FDRs
       ...,
       flatOutput = FALSE,
  ...
  selectASComp = if (haveReduced) as.numeric(reduced!="")+1 else rep(1, nDesigns);
   ...
   verbose = 2, indent = 0)
{
   ...
   if (useLimma)
   {
      ...
      modelComparisonSuffix = if (getCoeffSignificance &&!is.null(reduced)) ".modelComp" else 
                                  spaste(testNameSep, termOfInterest);
      screening = mtd.mapply(associationScreening,
             ...
             MoreArgs = list(
                 ...
                 getZstatistics = TRUE,
                 getDoF = TRUE,
                 getCoeffSignificance = getCoeffSignificance,
                 getCLS = getCLS,
                                  ...
                                  extraLabels = if (is.numeric(targetLabels)) c(1:1000) else standardColors())
{
  ...
  targetLabels.src = targetLabels[ match(sourceEntrez, targetEntrez)];
...
pValueStars2 = function(p)
{
  n = floor(-log10(p))
  n[ p>0.05] = 0;
  sapply(n, function(n1) paste(rep("*", n1), collapse = ""));
}
...
collectionFromModules = function(labels, identifiers, moduleInfo,
   ...
   IDBase,
   nameBase, descriptionPattern, source, 
   ...
   labelPrefix = "M")
{
   ...
   for (m in 1:nModules)
   {
     ...
     sets[[m]] = newGeneSet(
         geneEntrez = identifiers[labels==mm],
         geneEvidence = "HEP",
         geneSource = source,
...
rearrangeInternalCollections = function(collections)
{
  ...
  # WGCNA modules
  allWGCNA = subsetCollection(pool, c("WGCNA"), matchComponents = "groupsAndAlternates");
                         ...
                         cumulative = FALSE, ...)
{
  ...
  if (any(cumulative))
  {
     hists[cumulative] = lapply(hists[cumulative], function(h) {h$counts = cumsum(h$counts)/sum(h$counts); h})
     if (any(!cumulative))
                          ...
                          alternative = c("two.sided", "greater", "less"))
{
  ...
  nonSignif = replaceMissing(fdr > threshold);
  max(abs(Z[nonSignif]), na.rm = TRUE);
}
...
prettifyStrings = function(s, prettifyList, from=prettifyList[[1]], to = prettifyList[[2]])
{
  multiGSub(from, to, s, fixed = TRUE);
}
...
prettifyNames = function(df, prettifyList, from=prettifyList[[1]], to = prettifyList[[2]])
{
  names(df) = multiGSub(from, to, names(df), fixed = TRUE);
  df;
}

prettifyColumns = function(df, columns, prettifyList, from=prettifyList[[1]], to = prettifyList[[2]])
{
  ...
  df[columns] = lapply(df[columns], function(x) multiGSub(from, to, x, fixed = TRUE));
  df;
}
...
)
{
  ...
  enrichmentBarplot.general(
     ...
     classGenes = if (printGenes) analysis$topEntrezForEnrichment[[plotComponent]] else NULL,
     geneToSymbolTranslation = analysis$combinedDESeqResults[[plotComponent]] [c("Entrez", "Symbol")],
...
)
{
  ...
  ## FIXME: perhaps we should not prettify the data set names - those should be pretty as is.
                       ...
                       classBesideBars)
  {
    ...
    if (length(classGenes) > 0)
    {
      genes = formatLabels(spaste(tab1$nCommonGenes[rowOrder][rows], " genes: ", 
      ...
      xIsLogTransformed = FALSE, xLogBase = 2, xLogShift = 1, epsilon = 0.3)
{
  ...
  s1 = varianceSlope(log2(x+min));
  s2 = varianceSlope(log2(x + max));
  while(s2 < 0 && max < 1e6)
  {
    min = max;
    max = 2*max;
  ...
  while (max-min > epsilon)
  {
    ...
    sm = varianceSlope(log2(x + middle));
    if (s1*sm > 0)
    {
      s1= sm;
      min = middle;
  ...
  ...) # Arguments to plotOverlapHeatmap
{
  ...
  levels1 = rownames(ctab);
  levels2 = colnames(ctab);
  ...
  plotOverlapHeatmap(
    ...
    pTab = ot$pTable,
    colLabels = spaste("ME", labels2colors(as.numeric(levels2))),
    colSymbols = spaste(colLabelPrefix, levels2, if (includeSize) spaste( " (", n2, ")") else ""),
    rowLabels = spaste("ME", labels2colors(as.numeric(levels1))),
...
#==========================================================================================================
#
# Variacen and standard deviation of an estimate of variance
#
#==========================================================================================================
