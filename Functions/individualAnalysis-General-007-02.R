  ...
  for (dd in 1:nDesigns)
  {
    ...
    if (designs[d]==oldDesign && testTypes[d]=="Wald" && oldTestType=="Wald")
    {
      ...
      runArgs = c(list(object, test = testTypes[d]), 
                  DESeqArgs);
      if (testTypes[d]=="LRT") runArgs$reduced = as.formula(designInfo$reducedDesign[d]);
      deseq[[d]] = do.call(DESeq, runArgs);
      oldDesign = designs[d];
  ...
  order = FALSE)
{
  ...
  index = which(Z*sign >= 0 & p < pThreshold & pAdj < pAdjThreshold);
   ...
   )
{
   ...
   out = mymapply(function(res, addSplitBy1, addCombined1, testName1)
   { 
     Zs = list(Downregulated = res[, ZCol], Upregulated = -res[, ZCol]);
     logFCs = res[, FCCol];
     if (addCombined1) Zs = c(Zs, list(DE = -abs(res[, ZCol])));
     names(Zs) = spaste(c(DEDirectionNames, if (addCombined1) combinedName else NULL), testNameSep, 
     ...
     out;
   }, testResults, addSplitBy, addCombined, testNames);
...
individualAssociation.general = function(
   ...
   enrichment.minN = 100,
   enrichment.thresholds = topThresholds,
...
)
{
   ...
   analysisName = if (analysisName!="") make.names(analysisName) else "";
         ...
         (forceCalculation || is.null(intermediateResults$enrichment)))
   {
     ...
     topEntrez.enrichment = mymapply(function(tt, tName)
        suppressWarnings(getTopGenes(indivResults,
          ...
          pAdjThreshold = tt$pAdj,
          logFCThreshold = tt$logFC,
          topList.minN = enrichment.minN, geneIDs = geneAnnotation.start$Entrez,
          DEDirectionNames = DEDirectionNames,
          addSplitBy = designInfo$splitSignificantHitsBy,
          addCombined = designInfo$combineSignificantHits,
          testNameSep = testNameSep, returnFlatList = TRUE)), 
     ...
     topEntrez.comb.enrichment =  lapply(combineTestIndex.enr, function(i)
       interleaveLists.withoutNames(lapply(topEntrez.enrichment, function(ts) ts[i])));
     ...
     if (!is.null(geneSetFnc)) {
       ...
       {
          out = do.call(geneSetFnc, c(list(results = res, topList.minN = enrichment.minN,
                                      topThresholds = topThresholds,
                                      geneIDs = geneAnnotation.start$Entrez),
                                geneSetFncArgs));
          out;
       });

       topEntrez.ext.enrichment = mymapply(c, topEntrez.comb.enrichment, topOrNIDs.extraLists.enrichment);
     } else 
        topEntrez.ext.enrichment = topEntrez.comb.enrichment;

    # Drop length-zero gene lists
     topEntrez.enr = lapply(topEntrez.ext.enrichment, function(lst) lst[sapply(lst, length)>0]);
     ...
     for (cri in 1:nUseCombineResults)
     {
       ...
       enr1 = do.call(enrichmentAnalysis,
                   c(list(active = topEntrez.enr[[cr]], inactive = geneAnnotation.start$Entrez,
       ...
       enrichment[[cri]] = list(CombinedCollection = c(list( enrichmentTable = enrTable),
   ...
   {
     dir.create(enrichmentDir, recursive = TRUE);
     nUseCombineResults = length(enrichment.useCRIndex);
     for (cri in 1:nUseCombineResults) 
     {
       cr = enrichment.useCRIndex[cri];
       enr.subColl = splitEnrichmentTableBySubcollections(enrichment[[cri]]$CombinedCollection$enrichmentTable, 
                       enrichment.combinedCollection, enrichment.collections, dropColumns = enrichment.dropCols);
       enrichment.temp = c(list(enrichment[[cri]]$CombinedCollection$enrichmentTable), enr.subColl);
       for (col in 1:nCollections) if (nrow(enrichment.temp[[col]]) > 0)
       {
         f = file.path(enrichmentDir, spaste(enrichmentFileBase, "-", combineNames[cr],
                                  "-", prependZeros(col-1, 2), "-", collectionNames[col], ".csv"));
         ...
         if (addToAnalysisManager) amAnalysis = am.addNewObjectToAnalysis(
            name = c("enrichment analysis", combineNames[cr], collectionNames[col]),
            fileName = f,
            analysis = amAnalysis,
            anaType = amComponent,
            allowNonstandardAnalysisType = TRUE,
