                          ...
                          combineSignificantHits = FALSE)
{
   ...
   if (!all)
   {
     #keepMat = matrix( make.names(c(background, wt, 
     #                 pertBack, background,
     #                 pertBack, wt)),
     #                 ncol = 2, byrow = TRUE);
     #keepNames = spaste( testNameBase, apply(keepMat, 1, paste, collapse = levelSep));
     #keepIndex = match(keepNames, out$testName);
     keepIndex = c(1:3);
     if (any(is.na(keepIndex)))
       stop("Internal error: some keepNames are not among testNames.");
     out = out[keepIndex, ];
   }
   out;
}
...
GNV.addAgingWGCNAcollection = function(rootDir=NULL, organism = "mouse",
                               ...
                               addCombined = TRUE, combinedName = "AllCombined")
{
  ...
  {
    if (highKME) {
       x = loadAsList(file.path(rootDir, "Aging/AgingInWTMice-NanWang-2018-9096/030-NetworkAnalysis/RData",
          "moduleCollection-WTAging.Striatum-highKME.RData"));
    } else {
       x = loadAsList(file.path(rootDir, "Aging/AgingInWTMice-NanWang-2018-9096/030-NetworkAnalysis/RData",
          "moduleCollection-WTAging.Striatum.RData"));
    }
    x2 = loadAsList(file.path(rootDir, "Aging/AgingInWTMice-NanWang-2018-9096/020-IndividualAnalysis/RData",
          "collectionOfDEGenes-WTAgingStriatum.RData"));
    if (removeInteractionSets)
    {
      # Get rid of interaction sets...
      x2$assocCollection = subsetCollection(x2$assocCollection, tags = "interaction", exactMatch = FALSE, invertSearch = TRUE);
    }
    getDECollection = TRUE
  } else {
  ...
  if (getDECollection)
  {
    coll= mergeCollections(x[[1]], x2$assocCollection);
  } else coll = x[[1]];
  ...
  collections = c(collections,
        list(AgingWGCNA = convertCollectionToOrganism(coll, organism = organism)));
...
GNV.addWTGNVModuleCollection = function(
   rootDir=NULL, organism = "mouse",
   highKME = FALSE,
   collections = NULL,
   addCombined = TRUE, combinedName = "AllCombined")
{
  ...
  coll = loadAsList(spaste(rootDir, "/HuntingtonsDisease/IndividualAnalyses/GNVProject/CommonAnalyses/",
          "160-WGNCA-perGenotype/030-NetworkAnalysis/RData/moduleCollection-WT", 
          if (highKME) "-stringentKME" else "", ".RData"))[[1]];

  collections = c(collections,
        list(WT.GNV.WGCNA = convertCollectionToOrganism(coll, organism = organism)));
...
GNV.addCircadianWGCNAcollection = function(rootDir=NULL, organism = "mouse",
                               getDECollection = TRUE,
                               tissue = "striatum", 
                               highKME = FALSE,
                               defaultTissue = NULL,
                               ...
                               addCombined = TRUE, combinedName = "AllCombined")
{
  ...
  if (highKME)
  {
     x = loadAsList(spaste(rootDir, 
          "/HuntingtonsDisease/IndividualAnalyses/500-CHDI-CircadianGeneExpression/030-NetworkAnalysis/RData",
          "/moduleCollection-selected-", availableTissues[tsIndex], ".RData"));
  } else {
     x = loadAsList(spaste(rootDir, 
          "/HuntingtonsDisease/IndividualAnalyses/500-CHDI-CircadianGeneExpression/030-NetworkAnalysis/RData",
          "/moduleCollection-selected-", availableTissues[tsIndex], "-highKME.RData"));
  }
  x2 = loadAsList(spaste(rootDir, 
  ...
  # Select subset
  x2$assocCollection = subsetCollection(x2$assocCollection, tags = keepSV[tsIndex], exactMatch = FALSE);
  ...
  ...)
{
  ...
  if (separateLegend && plotLegend && !is.null(colorFactor))
  {
    ...
    {
      shape = 20 + as.numeric(as.factor(shapeFactor));
    ...
    out = legendClean("top",
              ...
              pch = c(rep(21, nCFLevels), 20+seq_len(nShapeLevels)),
              pt.bg = labels2colors(c(1:nCFLevels, rep(27, nShapeLevels))),
              col = 1);
  ...
  ...)
{
  ...
  if (!is.null(genotype))
  {
    ...
    bgColors = labels2colors(geno.num);
  } else {
    bgColors = pt.bg;
    genoLevels = NULL;
  }
  ...
  {
    borderColors = as.numeric(factor(borderFactor));
  } else
    borderColors = 1;
  ...
  plotFnc(svd$u[, 1], svd$u[, 2],
       ...
       bg = bgColors,
       col = borderColors, lwd = if (is.null(borderFactor)) 1 else 1,
       ...)
  ...
  if (plotLegend && nGenoLevels > 0)
    legendClean("auto", points.x = svd$u[, 1], points.y = svd$u[, 2],
              ...
              pt.cex = min(pt.cex, 1.5),
              pch = c(rep(21, nGenoLevels), 20+seq_len(nShapeLevels)), 
              pt.bg = labels2colors(c(1:nGenoLevels, rep(27, nShapeLevels))),
              col = 1);
  ...
  cex = 1)
{
  ...
  pValues = res[, grep(pPattern, colnames(res))];
...
GNV.rescuePlotAttributes = function(analysis,
   ...
   results = analysis$combinedDESeqResults[[1]],
   ...
   sigExacName = spaste(sigExacColBase, sigThreshold))
{
  ...
  if (is.null(rescueScoreCol))
  {
    ...
    rescueInd = res[[sigRescName]];
    exacInd = res[[sigExacName]];
  }
  ...
  ...)
{
  ...
  data = analysis$combinedDESeqResults[[1]];
...
enrichmentPlot.barplot = function(
   analysis,
...
)
{
  ...
  stop("Use enrichmentBarplot.standardAnalysis instead.");
  enrComp = spaste(analysisName, ".", make.names(plotComponent));
  enrTab = analysis$enrichment[[ enrComp ]] [[collectionName]]$enrichmentTable
  ...
  if (rescueOnly) keepRows = grepl("Rescue|Exacerbation", enrTab$class) & keepRows;

  if (!is.null(dropClassPattern))
  ...
  setNames0 = tab1$shortDataSetName;
  ...
  if (dropModuleEnrichmentLabels) 
  {
    ...
    if (any(isModule)) {
      setNames[isModule] = spaste("M", moduleLabels[isModule]);
    } else 
  ...
  # enrichmentAnalysis.
  classGenes = translateUsingTable(
               analysis$topEntrezForEnrichment[[spaste(analysisName, ".", plotComponent)]] [tab1$class],
               analysis$combinedDESeqResults[[spaste(analysisName, ".", plotComponent)]] [c("Entrez", "Symbol")]);
  overlapGenes = mymapply(intersect,  classGenes, overlapGenes0);
  overlapGenes.ps = sapply(overlapGenes, paste, collapse = ", ");

  rowOrder = c(grep("Downregulated|down.for", tab1$class), grep("Upregulated|up.for", tab1$class));

  yLabels = prettifyStrings.multi(setNames[rowOrder], list(analysis$prettifyList.plots, prettifyList));

  capitalize = function(s) { substr(s,1,1) = toupper(substr(s, 1,1)); s }
  classes = unique(tab1$class[rowOrder]);
  classes.pretty = capitalize(prettifyStrings.multi(classes, list(analysis$prettifyList.plots, prettifyList)));
  if (splitClassOnAnd) classes.pretty = sub(" and ", "\nand ", classes.pretty);

  lp = -log10(tab1$Bonferroni[rowOrder]);

  lim = (floor( max(c(lp, 1))) / floor(max(c(lp/4, 1))) +1) * floor(max(c(lp/4,1)));
  if (lim > axisLimit)
  {
    lim = axisLimit;
  }
  ...
  maxRP = max(sapply(rowsPerPage, length));
  if (!is.null(plotDevice))
  {
    device = match.fun(plotDevice);
  ...
  plotAnnot = function(plotCoords, 
                       ...
                       classBesideBars)
  {
    abline(v = -log10(0.05), col = "#FFCCCC", lwd =2)
    ...
    if (printGenes)
    {
      genes = formatLabels(spaste(tab1$nCommonGenes[rowOrder][rows], " genes: ", 
                           ...
                           maxWidth = 0.96 * lim, maxLines = maxLines.genes);
  ...
  bp = labeledBarplot3.multiPage(lp,
               ...
               lim = c(0, lim),
               formatYLabels = TRUE,
               yLabels.maxFracExtWidth = if (classBesideBars) 0.57 else 0.98,
